...
def non_empty(text, fallback, stub="See the overview on MedlinePlus."):
    t = (text or "").strip()
    if t:
        return t
...
def main():
    ...
    ap.add_argument("--out", default="medline_eval_969_v2.jsonl")
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--per_title_cap", type=int, default=3)   # 同一頁面最多幾條
    ap.add_argument("--per_term_page_cap", type=int, default=10)  # 同一term最多取幾頁
    args = ap.parse_args()
    ...
    with open(args.out, "w", encoding="utf-8") as f:
        for term in TOPICS:
            ...
            # 打亂後抽前 K 頁，提升可重現性與可控性
            docs = list(docs)
            random.shuffle(docs)
            docs = docs[:args.per_term_page_cap]
            ...
            for doc in docs:
                ...
                # 解析出頁面 title / FullSummary，做保底
                contents = parse_document_contents(doc)
                full_summary = clean_text(contents.get(
                    "FullSummary") or contents.get("full-summary") or "")

                written_for_this_page = 0
                for item in qa_items:
                    # qtype 與保底
                    item["qtype"] = qtype_from_q(item["question"])
                    item["answer"] = trim_words(
                        non_empty(item["answer"], full_summary), 120)
                    item["dataset"] = "969_v2"

                    # 去重 key（依需求可只用 question+answer）
                    key = hashlib.md5(
                        (item["topicTitle"]+"||"+item["question"]+"||"+item["answer"]).encode("utf-8")).hexdigest()
                    if key in seen:
                        continue
                    seen.add(key)

                    # 每頁上限
                    if written_for_this_page >= args.per_title_cap:
                        break

                    # 長度保關（與你現有 12/120 規則一致）
                    if len(item["answer"].split()) < 12:
                        continue

                    f.write(json.dumps(item, ensure_ascii=False) + "\n")
                    n_total += 1
                    written_for_this_page += 1

            time.sleep(0.2)

    print(f"Done. Wrote {n_total} QA items to {args.out}")
